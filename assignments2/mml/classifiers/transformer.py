...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        embeddings = self.embedding(captions)  # Shape: (N, T, W)

        # 2. Add positional encoding
        embeddings = self.positional_encoding(embeddings)  # Shape: (N, T, W)

        # 3. Project image features to the same dimension as word vectors
        memory = self.visual_projection(features)  # Shape: (N, W)
        memory = memory.unsqueeze(1)  # Shape: (N, 1, W) to match Transformer memory shape

        # 4. Create a target mask to prevent attending to future positions
        # The mask shape should be (T, T)
        tgt_mask = torch.tril(torch.ones(T, T, device=captions.device)).bool()  # Shape: (T, T)

        # 5. Pass embeddings and memory through the Transformer decoder
        transformer_output = self.transformer(embeddings, memory, tgt_mask=tgt_mask)  # Shape: (N, T, W)

        # 6. Project the Transformer output to vocabulary size to get scores
        scores = self.output(transformer_output)  # Shape: (N, T, V)
